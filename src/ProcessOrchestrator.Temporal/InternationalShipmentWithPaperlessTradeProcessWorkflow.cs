...
[Workflow]
public class InternationalShipmentWithPaperlessTradeProcessWorkflow
{
    [WorkflowRun]
    public async Task<ShipmentProcessResult> Run(ShipmentProcessRequest request)
    {
        var ptRequest = new PaperlessTradeDocumentsGenerationRequest
        {
        ...
        var ptResult = await Workflow.ExecuteChildWorkflowAsync(
            (PaperlessTradeDocumentsGenerationWorkflow wf) =>
                wf.GeneratePaperlessTradeDocuments(ptRequest),
        ...
        string ptDocumentUrl;
        switch (ptResult._Case)
        {
            case nameof(PaperlessTradeDocumentsGenerationResult.Success):
        ...
        var shipmentManifestationRequest = new ShipmentManifestationRequest
        {
            ...
            CollectionDate = request.CollectionDate,
            TimeZone = request.TimeZone,
            PaperlessTradeDocumentUrl = ptDocumentUrl
        };
